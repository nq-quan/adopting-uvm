...
task uvm_reg::do_read(uvm_reg_item rw);
   ...
   case (rw.path)
      ...
      UVM_BACKDOOR: begin
         ...
         if (map.get_check_on_read()) exp = get_mirrored_value(); // CAVM
      ...
      UVM_FRONTDOOR: begin
         ...
         if (rw.local_map.get_check_on_read()) exp = get_mirrored_value(); // CAVM
...
function bit uvm_reg::do_check(input uvm_reg_data_t expected,
   ...
   // CAVM vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
   if ((actual|dc) === (expected|dc))  
      return 1; 
   else begin 
      
      // Cavium specific change, make one error message for the register miscompare,  
      // but it is multiple lines with %E- prefixed on the additional lines 
      string                         error_msg; 
      // indent_str is prefixed tothe field specifics line 
      string                         indent_str; 
      // make the indent_str by concat'ing a number of a specific char, to make it easy to change the len and character 
      indent_str = {(3){"_"}}; 
      
      // jsweeney, the original UVM error message covered up reason for a miscompare as it put an X in the nibble with the error 
      error_msg = $sformatf("Register \"%s\" value read from DUT does not match mirrored value ", get_full_name()); 
      error_msg = {error_msg,  
	           $sformatf("(read=0x%h)(mirr=0x%h)(mask=0x%h)\n", 
	                     actual, expected, dc)}; 
      
      foreach(m_fields[i]) begin 
	 string acc = m_fields[i].get_access(map); 
	 acc = acc.substr(0, 1); 
	 if (! (m_fields[i].get_compare() == UVM_NO_CHECK ||  // CAVM 
	        m_fields[i].get_access() == "WO")) begin // CAVM 
	    ...
	    if (val !== exp) begin 
	       // concatenate the specific fields which miscompare, one field per line 
	       error_msg = {error_msg,  
	                    $sformatf("%sField %s (%s[%0d:%0d]) mismatch read=%0d'h%0h mirrored=%0d'h%0h\n", 
	                              indent_str, 
